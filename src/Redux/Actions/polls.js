...
export function listPolls() {
  return (dispatch, getState) => {
    ...
    return _getQuestions()
      .then((polls) => {
        dispatch({ type: POLLS_LIST, polls });
...
export function savePoll(payload) {
  return (dispatch, getState) => {
    ...
    return _saveQuestion({
      ...
      .then((poll) => {
        const info = { ...user, questions: user.questions.concat([poll.id]) };
        dispatch({ type: ADD_POLL, poll });
        dispatch({ type: UPDATE_USER, info });
        dispatch({ type: SUCCESS, message: "Poll saved successfully" });
      })
      .catch((err) => {
        dispatch({ type: FAIL, message: `An error occurred ::: ${err.message}`, });
      });
  };
}
